...
async function handleEtaSearch(query) {
  ...
  try {
    // 1. Make the HTTP GET request to your backend URL
    // encodeURIComponent ensures spaces/special characters are URL-safe
    const response = await fetch(`${API_BASE}/api/trains?search=${encodeURIComponent(normalized)}`);
    ...
    // 3. Convert the response to a usable JavaScript object.
    // app.py wraps the train record as { success, data: {...} }, so we
    // need data.data — the previous version rendered the envelope itself.
    ...
    const trainData = envelope.data;

    // 4. Fetch the ML-predicted delay/ETA for this train and merge it into
    // the existing eta-panel fields (delayMinutes / nextEtaTime) so it shows
    // up in the UI exactly where live delay/ETA already render.
    ...
    // 5. Handle network failures (e.g., user is offline, server is down)
    console.error("Network error while fetching train data:", error);
    renderNotFound(query);
    return null;
  }
...
async function attachPrediction(trainData) {
  const trainNumber = trainData.number || trainData.train_number;
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/api/predict`, {
      ...
      body: JSON.stringify({ train_number: trainNumber, date: today })
    });

    if (!response.ok) {
      console.warn("Prediction unavailable for this train, showing live data only");
      return;
    }
    ...
    if (typeof prediction.predicted_delay === "number") {
      trainData.delayMinutes = prediction.predicted_delay;
    }
    if (prediction.predicted_arrival_time) {
      trainData.nextEtaTime = prediction.predicted_arrival_time;
    }
  } catch (error) {
    console.warn("Prediction request failed, showing live data only:", error);
  }
}
