...
CORS(
    ...
    resources={
        r"/api/*": {
            "origins": [
                "https://gati-drishti.vercel.app",
                r"https://gati-drishti-[a-z0-9]+-nexora-hk\.vercel\.app",
                "http://127.0.0.1:5500",
                "http://localhost:5500"
            ]
        }
...
# CORS SAFETY NET
# Adds the CORS headers for the same allowed origins if Flask-CORS did
# not (also on error responses, so the browser shows the real error
# instead of an opaque "blocked by CORS policy").
import re as _re
...
_CORS_PREVIEW_ORIGIN = _re.compile(
    r"https://gati-drishti-[a-z0-9]+-nexora-hk\.vercel\.app"
)
...
def ensure_cors_headers(response):
    ...
    if (
        ...
        and (
            origin in _CORS_EXACT_ORIGINS
            or _CORS_PREVIEW_ORIGIN.fullmatch(origin)
        )
    ):
...
def health():
    ...
    return jsonify({
        ...
        # Marker: proves this build has the frontend-weather changes.
        "frontend_weather_support":
            True
    })
